...
namespace Utils {
    public class Math3d {
        ...
        public static bool PlanePlaneIntersection(out Vector3 lineVec, Vector3 plane1Normal, Vector3 plane1Position, Vector3 plane2Normal, Vector3 plane2Position) {
            Vector3 point;
            return PlanePlaneIntersection(out point, out lineVec, plane1Normal, plane1Position, plane2Normal, plane2Position);
        }

        //Get the intersection between a line and a plane. 
        //If the line and plane are not parallel, the function outputs true, otherwise false.
        public static bool LinePlaneIntersection(out Vector3 intersection, out float length, Vector3 linePoint, Vector3 lineVec, Vector3 planePoint, Vector3 planeNormal) {
            float dotNumerator;
        ...
        public static bool LinePlaneIntersection(out Vector3 intersection, Vector3 linePoint, Vector3 lineVec, Vector3 planePoint, Vector3 planeNormal) {
            float length;
            return LinePlaneIntersection(out intersection, out length, linePoint, lineVec, planePoint, planeNormal);
        }
        ...
        public static bool LineTriangleIntersection(Vector3 p1, Vector3 p2, Vector3 p3, Ray ray) {
        ...
        // IMAX GAMES FUNCTIONS

        public static int LineSphereIntersection(out Vector3 intersection1, out Vector3 intersection2, Vector3 linePoint, Vector3 lineVec, Vector3 sphereCenter, float radius) {
            intersection1 = Vector3.zero;
            intersection2 = Vector3.zero;

            Vector3 p = linePoint - sphereCenter;
            float rSquared = radius * radius;
            float pd = Vector3.Dot(p, lineVec);
            float d = pd * pd - p.sqrMagnitude + rSquared;

            if (d < 0)
                return 0;
            if (d < float.Epsilon && d > -float.Epsilon) {
                intersection1 = linePoint - pd * lineVec;
                return 1;
            }
            intersection1 = linePoint - (pd + Mathf.Sqrt(d)) * lineVec;
            intersection2 = linePoint - (pd - Mathf.Sqrt(d)) * lineVec;
            return 2;
        }

        /// <summary>
        /// Cartesian (X, Y, Z) to Sphere (R, Theta[-PI/2,PI/2], Phi[-PI,PI]) coordinate system
        /// </summary>
        /// <param name="cart">Cartesian X, Y, Z</param>
        /// <returns>Sphere R, Theta[-PI/2,PI/2], Phi[-PI,PI]</returns>
        public static Vector3 Cart2Sphere(Vector3 cart) {
            float radius = cart.magnitude;
            float theta = Mathf.Asin(cart.y / radius);
            float phi = Mathf.Atan2(cart.z, cart.x);
            return new Vector3(radius, theta, phi);
        }

        /// <summary>
        /// Sphere (R, Theta[-PI/2,PI/2], Phi[-PI,PI]) to Cartesian (X, Y, Z) coordinate system
        /// </summary>
        /// <param name="sphere">Sphere R, Theta[-PI/2,PI/2], Phi[-PI,PI]</param>
        /// <returns>Cartesian X, Y, Z</returns>
        public static Vector3 Sphere2Cart(Vector3 sphere) {
            float x = sphere.x * Mathf.Cos(sphere.y) * Mathf.Cos(sphere.z);
            float z = sphere.x * Mathf.Cos(sphere.y) * Mathf.Sin(sphere.z);
            float y = sphere.x * Mathf.Sin(sphere.y);
